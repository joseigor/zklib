...
void test_zk_vector_data_should_return_not_NULL_when_vector_is_empty(void)
{
	zk_vector *vec = zk_vector_new();
	TEST_ASSERT_NOT_NULL(vec);
	TEST_ASSERT_NOT_NULL(zk_vector_data(vec));
...
int main(void)
{
	...
	RUN_TEST(test_zk_vector_data_should_return_not_NULL_when_vector_is_empty);
	return UNITY_END();
}
