...
int main()
{
	...
	list = zk_slist_push_back(list, &custom_node_1);
	list = zk_slist_push_back(list, &custom_node_2);
	list = zk_slist_push_back(list, &custom_node_3);

	// use your list ...

	// in this case there is no need to free the memory the list node are pointing to because they are pointing to
	// static allocated memory (no-heap object) so func argument of zk_list_free() can be NULL
